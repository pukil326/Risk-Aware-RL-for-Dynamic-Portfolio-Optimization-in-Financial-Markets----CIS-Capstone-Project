...
# Changed output directory name for PPO
OUTDIR_DEFAULT = "ml_outputs_ppo_monthly"
...
def monthly_split_data(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Splits data into training (odd months) and testing (even months) sets.
    """
    print("[info] Splitting data by month: Train on odd months, Test on even months.")
    
    if not isinstance(df.index, pd.DatetimeIndex):
        raise TypeError("Dataframe index must be a DatetimeIndex for monthly split.")
        
    train_mask = df.index.month % 2 != 0  # Odd months: 1, 3, 5, 7, 9, 11
    test_mask = ~train_mask               # Even months: 2, 4, 6, 8, 10, 12
    
    return df[train_mask].copy(), df[test_mask].copy()

# ========== Main Runner ==========
def run(symbol="AAPL", episodes=50, outdir=OUTDIR_DEFAULT):
    print(f">>> Advanced PPO Agent (Monthly Split)  Py={sys.executable}", flush=True)
    ...
    train_df, test_df = monthly_split_data(df)
    ...
    if len(X_tr) == 0 or len(X_te) == 0:
        print("[error] Not enough data in monthly splits to train and test. Exiting.")
        return
    ...
    # -------- Training --------
    print("[info] Starting Advanced PPO training...", flush=True)
    n = len(X_tr)
    ...
    strategies = {'PPO_Advanced': actions, **ta_strats, 'Buy_and_Hold': np.ones_like(actions)}
    ...
    # Changed pnl column name
    sim_df[f'pnl_PPO_Advanced'] = sim_df['portfolio_value_PPO_Advanced'] - initial_cash
...
if __name__ == "__main__":
    import argparse
    # Updated description
    ap = argparse.ArgumentParser(description="Advanced PPO Trading Agent with Monthly Split")
