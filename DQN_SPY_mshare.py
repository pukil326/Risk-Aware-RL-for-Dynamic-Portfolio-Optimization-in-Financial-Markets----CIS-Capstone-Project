...
# Full feature list from indicators pipeline
STATE_FEATURES = [
...
# --- New Realistic CASH-BASED Simulation Function ---
def simulate_strategy(
    prices: np.ndarray, 
    actions: np.ndarray, 
    positions_map: Dict[int, float], # e.g., {0: 0.0, 1: 0.5, 2: 1.0}
    initial_cash: float, 
    txn_cost: float
) -> np.ndarray:
    """
    ...
    for i in range(len(prices) - 1):
        ...
        if shares_to_trade > 0: # --- WANT TO BUY ---
            ...
            if cash >= cash_needed:
                ...
                # This is the "buy more as long as it's not exceed my total money left" logic
        ...
        elif shares_to_trade < 0: # --- WANT TO SELL ---
            # We want to sell `abs(shares_to_trade)`
            # We can't sell more shares than we own
            actual_shares_to_sell = min(abs(shares_to_trade), shares)
            
            if actual_shares_to_sell > 0:
                cash_gained = actual_shares_to_sell * current_price * (1.0 - txn_cost)
...
def run(symbol="SPY", episodes=30, outdir=OUTDIR_DEFAULT, csv_dir="data"):
    ...
    # ------- Agent -------
    # --- Define position mapping and action size ---
    # We allow the agent to choose between 3 positions:
    # Action 0 -> 0% invested (Flat)
    # Action 1 -> 50% invested (Half-Long)
    # Action 2 -> 100% invested (Full-Long)
    ...
    for ep in range(1, episodes + 1):
        # --- Update training loop for new position logic ---
        pos = 0.0  # Start flat (0.0 = 0%)
        state = np.concatenate([X_tr[0], [pos]]).astype(np.float32)

        for t in trange(n - 1, desc=f"Episode {ep}/{episodes}"):
            action = agent.choose_action(state)  # 0, 1, or 2
            next_pos = POSITIONS[action] # Target position (0.0, 0.5, or 1.0)
            ...
            # Action 'action' (0, 1, or 2) led to this
    ...
    # --- CHANGE 4: Generate DQN actions using new logic ---
    ...
    for t in range(len(X_te)): # Iterate all the way to generate all actions
        ...
        a = agent.choose_action(s, greedy=True) # 0, 1, or 2
        actions_dqn[t] = a
    ...
    # ---`simulate_strategy` function for ALL strategies ---
    ...
    # --- Update trading log for 3-state actions ---
    aa = strategies["DQN_Advanced"] # Actions are 0, 1, 2
    position_labels = {0: "FLAT", 1: "HALF-LONG", 2: "FULL-LONG"}
    ...
    log_df = pd.DataFrame({
        ...
        "position": [position_labels[a] for a in aa],
